    ...
    def make_plots_for_all_files(self) -> None:
        ...
        for file in self.files:
            if file.endswith(".xlsx"):
                ...
                # self.make_plot_for_excel_file(filepath)
            # filepath = os.path.join(self.folderpath, self.files[1])
            # print("Making plots for file: ", filepath)
            # self.make_plot_for_file(filepath)

    def make_plot_for_excel_file(self, filepath: str = None) -> None:
        """
        ...
        def make_subplot(ax, value, title, step: int = -1) -> None:
            ...
            ax.axvline(df['ElapsedSeconds'][step], color='r', linestyle='--', linewidth=2)
